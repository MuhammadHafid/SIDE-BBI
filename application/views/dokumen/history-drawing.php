                ...
                <tbody>
                  ...
                  <?php foreach ($dokumen_history as $dh): ?>
                    <tr>
                      ...
                      <td style="font-size: 14px;"><?= $dh['no_sp']; ?></td>                      
                      <td style="font-size: 14px;"><?= $dh['nama_dokumen']; ?></td>
                      <td style="font-size: 14px;"><?= $dh['tgl_dokumen']; ?></td>
                      <td style="font-size: 14px;"><?= $dh['nama_status']; ?></td>
                      <td style="font-size: 14px;"><?= $dh['nama_edisi']; ?></td>
                      <td style="font-size: 14px;"><?= $dh['nama_revisi']; ?></td>
                      <!-- <td style="font-size: 14px;"><?= $dh['issue_sheet']; ?></td> -->
                      ...
                      <td>
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            ...
                            <!-- view -->
                            <a class="dropdown-item" target="_blank" href="<?= base_url('drawing/view/').$dh['nama_file'] ?>">View</a>
                            <!-- view -->

                            <?php if ($this->session->userdata('id_role') == 1 OR $this->session->userdata('id_role') == 5): ?>
                            <!-- hapus -->
                            <a href="<?= base_url('drawing/hapus/') . $dh['id_dokumen'].'/'.$no_order.'/'.$dh['nama_file'] ?>" class="dropdown-item" onclick="return confirm('Yakin akan menghapus ini?')">Hapus</a>
                            <!-- hapus -->  
                          <?php endif ?>
