...
class Mscurve extends CI_Controller
{
    ...
    public function view($nama_file)
    {
        ...
        $jenis_dokumen = 'Purchase Requisition Sheet';
        $tgl_view = date('Y-m-d');
        ...
        $this->load->view('dokumen/view-pr', $data);
    }
    // ///////////////////////////////////////////////////// view dokumen pr //////////////////////////////////////////////////////
    ...
    public function tambah($no_ord, $fungsi = 'nol')
    {
        ...
        // file pr
        $file = $_FILES['file_pr']['name'];

        // cek apakah nomor dokumen sudah terdaftar ?
        $cekNoDok = $this->db->get_where('dokumen_pr', ['no_dokumen' => $no_dokumen])->num_rows();
        ...
        if ($file) {
            ...
            $nama_file = $this->dokumen->tambahPr($file);
        }
        // file pr
        ...
        $this->db->insert('dokumen_pr', $data);

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Purchase Requisition Sheet berhasil ditambahkan!</div>');
    ...
    public function edit($id_dokumen, $fungsi = 'nol')
    {
        ...
        $fd = $this->db->get_where('dokumen_pr', ['id' => $id_dokumen])->row_array();
        // ambil inputan
        ...
        if ($file) {
            $this->load->model('Dokumen_model', 'dokumen');
            $nama_file = $this->dokumen->editPr($fd['nama_file'], $file);
        ...
        $this->db->update('dokumen_pr');

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Purchase Requisition Sheet berhasil diubah!</div>');
    ...
    public function revisi($kode_unik, $no_order, $fungsi = 'nol')
    {
        ...
        // ambil dokumen pr where kode unik
        $this->db->select('dokumen_pr.*, nama_status, nama_edisi, nama_revisi, judul_dokumen');
        $this->db->from('dokumen_pr');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_pr.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_pr.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_pr.revisi', 'left');
        $this->db->join('distribusi_dokumen', 'distribusi_dokumen.id_dd = dokumen_pr.id_dd', 'left');
        $this->db->where('dokumen_pr.kode_unik =', $kode_unik);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('dokumen/revisi-pr', $data);
            ...
            // cek jumlah dokumen pr where no_dokumen dan status valid
            $this->db->select('*');
            $this->db->from('dokumen_pr');
            ...
            if ($cDV == 0) {
                ...
                // insert dokumen pr baru berdasarkan revisi
                $date_created = date('Y-m-d');
                $user = $this->session->userdata('nama_karyawan');
                $kode_unik = sha1(time());
                ...
                $this->load->model('Dokumen_model', 'dokumen');
                $nama_file = $this->dokumen->revisiPr($file);
            ...
            $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Purchase Requisition Sheet berhasil direvisi!</div>');
            redirect('order/dokumen/' . $no_order . '/' . $fungsi);
        }
    }
    // ///////////////////////////////////////////////// revisi dokumen pr ///////////////////////////////////////////////////////
    ...
    public function history($kode_unik, $no_order)
    {
        ...
        // ambil dokumen pr where kode unik
        $d = $this->db->get_where('dokumen_pr', ['kode_unik' => $kode_unik])->row_array();
        $no_dokumen = $d['no_dokumen'];

        $data['no_order'] = $no_order;

        // ambil dokumen pr where no dokumen, status
        $this->db->select('dokumen_pr.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_pr');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_pr.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_pr.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_pr.revisi', 'left');
        $this->db->where('dokumen_pr.no_dokumen =', $no_dokumen);
        $this->db->where('dokumen_pr.status =', 0);
        $data['dokumen_history'] = $this->db->get()->result_array();

        $this->load->view('template/sidebar', $data);
        $this->load->view('template/header', $data);
        $this->load->view('dokumen/history-pr', $data);
        $this->load->view('template/footer', $data);
    }
    // //////////////////////////////////////////////// history dokumen pr ///////////////////////////////////////////////////////
    ...
    public function viewer($nama_file)
    {
        ...
        // ambil dokumen_pr where nama_file
        $this->db->select('dokumen_pr.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_pr');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_pr.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_pr.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_pr.revisi', 'left');
        $this->db->where('dokumen_pr.nama_file =', $nama_file);
        ...
        $this->db->from('dokumen_view');
        $this->db->where('nama_file =', $nama_file);
        $this->db->where('jenis_dokumen =', 'Purchase Requisition Sheet');
    ...
    public function hapus($id_dokumen, $no_order, $nama_file, $fungsi = 'nol')
    {
        ...
        $this->db->where('id =', $id_dokumen);
        $this->db->delete('dokumen_pr');

        if ($nama_file != '' or $nama_file != NULL) {

            if (file_exists('assets/file/pr/' . $nama_file)) {
                unlink(FCPATH . 'assets/file/pr/' . $nama_file);
            }
        }

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Purchase Requisition Sheet berhasil dihapus!</div>');
