...
class Spk extends CI_Controller
{
    ...
    public function view($nama_file)
    {
        ...
        $this->load->view('dokumen/view-spk', $data);
    }
    // ///////////////////////////////////////////////////// view dokumen spk  //////////////////////////////////////////////////////
    ...
    public function tambah($no_ord, $fungsi = 'nol')
    {
        ...
        // file spk
        $file = $_FILES['file_spk']['name'];

        // cek apakah nomor dokumen sudah terdaftar ?
        $cekNoDok = $this->db->get_where('dokumen_spk', ['no_dokumen' => $no_dokumen])->num_rows();
        ...
        if ($file) {
            ...
            $nama_file = $this->dokumen->tambahSpk($file);
        }
        // file spk
        ...
        $this->db->insert('dokumen_spk', $data);

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen SPK berhasil ditambahkan!</div>');
    ...
    public function edit($id_dokumen, $fungsi = 'nol')
    {
        ...
        $fd = $this->db->get_where('dokumen_spk', ['id' => $id_dokumen])->row_array();
        // ambil inputan
        ...
        if ($file) {
            $this->load->model('Dokumen_model', 'dokumen');
            $nama_file = $this->dokumen->editSpk($fd['nama_file'], $file);
        ...
        $this->db->update('dokumen_spk');

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen SPK berhasil diubah!</div>');
    ...
    public function revisi($kode_unik, $no_order, $fungsi = 'nol')
    {
        ...
        $this->db->join('distribusi_dokumen', 'distribusi_dokumen.id_dd = dokumen_spk.id_dd', 'left');
        $this->db->where('dokumen_spk.kode_unik =', $kode_unik);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('dokumen/revisi-spk', $data);
            ...
            // cek jumlah dokumen spk where no_dokumen dan status valid
            $this->db->select('*');
            $this->db->from('dokumen_spk');
            ...
            if ($cDV == 0) {
                ...
                // insert dokumen spk baru berdasarkan revisi
                $date_created = date('Y-m-d');
                $user = $this->session->userdata('nama_karyawan');
                $kode_unik = sha1(time());
                ...
                $this->load->model('Dokumen_model', 'dokumen');
                $nama_file = $this->dokumen->revisiSpk($file);
            ...
            $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen SPK berhasil direvisi!</div>');
            redirect('order/dokumen/' . $no_order . '/' . $fungsi);
        }
    }
    // /////////////////////////////////////////////////// revisi dokumen spk ///////////////////////////////////////////////////////
    ...
    public function history($kode_unik, $no_order)
    {
        ...
        // ambil dokumen spk where kode unik
        $d = $this->db->get_where('dokumen_spk', ['kode_unik' => $kode_unik])->row_array();
        $no_dokumen = $d['no_dokumen'];

        $data['no_order'] = $no_order;

        // ambil dokumen spk where no dokumen, status
        $this->db->select('dokumen_spk.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_spk');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_spk.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_spk.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_spk.revisi', 'left');
        $this->db->where('dokumen_spk.no_dokumen =', $no_dokumen);
        $this->db->where('dokumen_spk.status =', 0);
        $data['dokumen_history'] = $this->db->get()->result_array();

        $this->load->view('template/sidebar', $data);
        $this->load->view('template/header', $data);
        $this->load->view('dokumen/history-spk', $data);
        $this->load->view('template/footer', $data);
    }
    // ////////////////////////////////////////////////// history dokumen spk ///////////////////////////////////////////////////////
    ...
    public function viewer($nama_file)
    {
        ...
        // ambil dokumen_spk where nama_file
        $this->db->select('dokumen_spk.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_spk');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_spk.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_spk.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_spk.revisi', 'left');
        $this->db->where('dokumen_spk.nama_file =', $nama_file);
        ...
        $this->db->from('dokumen_view');
        $this->db->where('nama_file =', $nama_file);
        $this->db->where('jenis_dokumen =', 'SPK');
    ...
    public function hapus($id_dokumen, $no_order, $nama_file, $fungsi = 'nol')
    {
        ...
        $this->db->where('id =', $id_dokumen);
        $this->db->delete('dokumen_spk');

        if ($nama_file != '' or $nama_file != NULL) {

            if (file_exists('assets/file/spk/' . $nama_file)) {
                unlink(FCPATH . 'assets/file/spk/' . $nama_file);
            }
        }

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen SPK berhasil dihapus!</div>');
