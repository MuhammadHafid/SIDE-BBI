...
class Mscurve extends CI_Controller
{
    ...
    public function view($nama_file)
    {
        ...
        $jenis_dokumen = 'Master Schedule Curve';
        $tgl_view = date('Y-m-d');
        ...
        $this->load->view('dokumen/view-mscurve', $data);
    }
    // ///////////////////////////////////////////////////// view dokumen mscurve  //////////////////////////////////////////////////////
    ...
    public function tambah($no_ord, $fungsi = 'nol')
    {
        ...
        $cekNoDok = $this->db->get_where('dokumen_mscurve', ['no_dokumen' => $no_dokumen])->num_rows();
        ...
        if ($file) {
            ...
            $nama_file = $this->dokumen->tambahMscurve($file);
        }
        // file mscurve
        ...
        $this->db->insert('dokumen_mscurve', $data);

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Master Schedule Curve berhasil ditambahkan!</div>');
    ...
    public function edit($id_dokumen, $fungsi = 'nol')
    {
        ...
        $fd = $this->db->get_where('dokumen_mscurve', ['id' => $id_dokumen])->row_array();
        // ambil inputan
        ...
        if ($file) {
            $this->load->model('Dokumen_model', 'dokumen');
            $nama_file = $this->dokumen->editMscurve($fd['nama_file'], $file);
        ...
        $this->db->update('dokumen_mscurve');

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Master Schedule Curve berhasil diubah!</div>');
    ...
    public function revisi($kode_unik, $no_order, $fungsi = 'nol')
    {
        ...
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_mscurve.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_mscurve.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_mscurve.revisi', 'left');
        $this->db->join('distribusi_dokumen', 'distribusi_dokumen.id_dd = dokumen_mscurve.id_dd', 'left');
        $this->db->where('dokumen_mscurve.kode_unik =', $kode_unik);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('dokumen/revisi-mscurve', $data);
            ...
            if ($cDV == 0) {
                ...
                // insert dokumen mscurve baru berdasarkan revisi
                $date_created = date('Y-m-d');
                $user = $this->session->userdata('nama_karyawan');
                $kode_unik = sha1(time());
                ...
                $this->load->model('Dokumen_model', 'dokumen');
                $nama_file = $this->dokumen->revisiMscurce($file);
            ...
            $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Master Schedule Curve berhasil direvisi!</div>');
            redirect('order/dokumen/' . $no_order . '/' . $fungsi);
        }
    }
    // ///////////////////////////////////////////////// revisi dokumen mscurve ///////////////////////////////////////////////////////

    // //////////////////////////////////////////////// history dokumen mscurve ///////////////////////////////////////////////////////
    public function history($kode_unik, $no_order)
    {
        ...
        $d = $this->db->get_where('dokumen_mscurve', ['kode_unik' => $kode_unik])->row_array();
        $no_dokumen = $d['no_dokumen'];

        $data['no_order'] = $no_order;

        // ambil dokumen mscurve where no dokumen, status
        $this->db->select('dokumen_mscurve.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_mscurve');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_mscurve.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_mscurve.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_mscurve.revisi', 'left');
        $this->db->where('dokumen_mscurve.no_dokumen =', $no_dokumen);
        $this->db->where('dokumen_mscurve.status =', 0);
        $data['dokumen_history'] = $this->db->get()->result_array();

        $this->load->view('template/sidebar', $data);
        $this->load->view('template/header', $data);
        $this->load->view('dokumen/history-mscurve', $data);
        $this->load->view('template/footer', $data);
    }
    // //////////////////////////////////////////////// history dokumen mscurve ///////////////////////////////////////////////////////

    // //////////////////////////////////////////////// viewer dokumen mscurve ////////////////////////////////////////////////////////
    public function viewer($nama_file)
    {

        // ambil dokumen_mscurve where nama_file
        $this->db->select('dokumen_mscurve.*, nama_status, nama_edisi, nama_revisi');
        $this->db->from('dokumen_mscurve');
        $this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_mscurve.status_dokumen', 'left');
        $this->db->join('edisi', 'edisi.id_edisi = dokumen_mscurve.edisi', 'left');
        $this->db->join('revisi', 'revisi.id_revisi = dokumen_mscurve.revisi', 'left');
        $this->db->where('dokumen_mscurve.nama_file =', $nama_file);
        ...
        $this->db->from('dokumen_view');
        $this->db->where('nama_file =', $nama_file);
        $this->db->where('jenis_dokumen =', 'Master Schedule Curve');
    ...
    // //////////////////////////////////////////////// hapus dokumen mscurve ////////////////////////////////////////////////////////
    public function hapus($id_dokumen, $no_order, $nama_file, $fungsi = 'nol')
    {

        $this->db->where('id =', $id_dokumen);
        $this->db->delete('dokumen_mscurve');

        if ($nama_file != '' or $nama_file != NULL) {

            if (file_exists('assets/file/mscurve/' . $nama_file)) {
                unlink(FCPATH . 'assets/file/mscurve/' . $nama_file);
            }
        }

        $this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen Master Schedule Curve berhasil dihapus!</div>');
