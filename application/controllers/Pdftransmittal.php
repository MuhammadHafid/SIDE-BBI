...
Class Pdftransmittal extends CI_Controller{
	...
	function index($id_transmittal, $no_order){
		...
		foreach ($drawing as $dr) {
			...
			if ($disdok['ppc'] != 0) {
				$pdf->Cell(8,5,$disdok['ppc'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}

			if ($disdok['qa'] != 0) {
				$pdf->Cell(8,5,$disdok['qa'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}

			if ($disdok['qc'] != 0) {
				$pdf->Cell(8,5,$disdok['qc'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}

			if ($disdok['fab'] != 0) {
				$pdf->Cell(8,5,$disdok['fab'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}

			if ($disdok['mm'] != 0) {
				$pdf->Cell(8,5,$disdok['mm'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}

			if ($disdok['log'] != 0) {
				$pdf->Cell(8,5,$disdok['log'].'x1','BRT',0,'C');
			}else{
				$pdf->Cell(8,5,'','BRT',0,'C');
			}
		...
		foreach ($mrs as $dmr) {
			...
			// ambil distribusi dokumen
			$disdok = $this->db->get_where('distribusi_dokumen',['id_dd'=>$dmr['id_dd']])->row_array();
