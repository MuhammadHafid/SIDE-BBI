...
class Mp extends CI_Controller
{
	...
	public function view($nama_file)
	{
		...
		$this->load->view('dokumen/view-mp', $data);
	}
	// ///////////////////////////////////////////////////////////// view dokumen MP ///////////////////////////////////////////////////////////////
	...
	public function tambah($no_order, $fungsi = 'nol')
	{
		...
		// file drawing
		$file = $_FILES['file_mp']['name'];

		// cek apakah nomor dokumen sudah terdaftar ?
		$cekNoDok = $this->db->get_where('dokumen_mp', ['no_dokumen' => $no_dokumen])->num_rows();
		...
		if ($file) {
			...
			$nama_file = $this->dokumen->tambahMP($file);
		}
		// file MP
		...
		$this->db->insert('dokumen_mp', $data);

		$this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen MP berhasil ditambahkan!</div>');
		redirect('order/dokumen/' . $no_order . '/' . $fungsi);
	}
	// //////////////////////////////////////////////////////////// tambah dokumen MP //////////////////////////////////////////////////////////////
	...
	public function edit($id_dokumen, $fungsi = 'nol')
	{
		...
		$fm = $this->db->get_where('dokumen_mp', ['id_dokumen' => $id_dokumen])->row_array();
		...
		if ($file) {
			...
			$this->load->model('Dokumen_model', 'dokumen');
			$nama_file = $this->dokumen->editMP($fm['nama_file'], $file);
		...
		$this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen MP berhasil diubah!</div>');
	...
	public function revisi($kode_unik, $no_order, $fungsi = 'nol')
	{
		...
		$this->db->join('distribusi_dokumen', 'distribusi_dokumen.id_dd = dokumen_mp.id_dd', 'left');
		$this->db->where('dokumen_mp.kode_unik =', $kode_unik);
		...
		if ($this->form_validation->run() == false) {
			...
			$this->load->view('dokumen/revisi-mp', $data);
			...
			// cek jumlah dokumen MP where no_dokumen dan status valid
			$this->db->select('*');
			$this->db->from('dokumen_mp');
			...
			if ($cDV == 0) {
				// insert dokumen MP baru berdasarkan revisi
				...
				$this->load->model('Dokumen_model', 'dokumen');
				$nama_file = $this->dokumen->revisiMP($file);
			...
			$this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen MP berhasil direvisi!</div>');
			redirect('order/dokumen/' . $no_order . '/' . $fungsi);
		}
	}
	// //////////////////////////////////////////////////////////// revisi dokumen MP //////////////////////////////////////////////////////////////
	...
	public function history($kode_unik, $no_order)
	{
		...
		// ambil dokumen MP where kode unik
		$d = $this->db->get_where('dokumen_mp', ['kode_unik' => $kode_unik])->row_array();
		$no_dokumen = $d['no_dokumen'];

		$data['no_order'] = $no_order;

		// ambil dokumen MP where no dokumen, status
		$this->db->select('dokumen_mp.*, nama_status, nama_edisi, nama_revisi');
		$this->db->from('dokumen_mp');
		$this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_mp.status_dokumen', 'left');
		$this->db->join('edisi', 'edisi.id_edisi = dokumen_mp.edisi', 'left');
		$this->db->join('revisi', 'revisi.id_revisi = dokumen_mp.revisi', 'left');
		$this->db->where('dokumen_mp.no_dokumen =', $no_dokumen);
		$this->db->where('dokumen_mp.status =', 0);
		$data['dokumen_history'] = $this->db->get()->result_array();

		$this->load->view('template/sidebar', $data);
		$this->load->view('template/header', $data);
		$this->load->view('dokumen/history-mp', $data);
		$this->load->view('template/footer', $data);
	}
	// /////////////////////////////////////////////////////////// history dokumen MP //////////////////////////////////////////////////////////////
	...
	public function viewer($nama_file)
	{
		...
		// ambil dokumen_mp where nama_file
		$this->db->select('dokumen_mp.*, nama_status, nama_edisi, nama_revisi');
		$this->db->from('dokumen_mp');
		$this->db->join('status_dokumen', 'status_dokumen.id_status = dokumen_mp.status_dokumen', 'left');
		$this->db->join('edisi', 'edisi.id_edisi = dokumen_mp.edisi', 'left');
		$this->db->join('revisi', 'revisi.id_revisi = dokumen_mp.revisi', 'left');
		$this->db->where('dokumen_mp.nama_file =', $nama_file);
		...
		$this->db->from('dokumen_view');
		$this->db->where('nama_file =', $nama_file);
		$this->db->where('jenis_dokumen =', 'MP');
	...
	public function hapus($id_dokumen, $no_order, $nama_file, $fungsi = 'nol')
	{
		...
		$this->db->where('id_dokumen =', $id_dokumen);
		$this->db->delete('dokumen_mp');

		if ($nama_file != '' or $nama_file != NULL) {

			if (file_exists('assets/file/mp/' . $nama_file)) {
				unlink(FCPATH . 'assets/file/mp/' . $nama_file);
			}
		}

		$this->session->set_flashdata('messageDokumen', '<div class="alert alert-success" role="alert">Dokumen MP berhasil dihapus!</div>');
